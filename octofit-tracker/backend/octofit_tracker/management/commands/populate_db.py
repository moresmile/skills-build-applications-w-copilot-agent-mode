...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Create teams
        teams = [
            Team(_id=ObjectId(), name='Blue Team', members=user_dicts[:2]),
            Team(_id=ObjectId(), name='Gold Team', members=user_dicts[2:]),
        ]
        ...
        activities = [
            ...
            Activity(_id=ObjectId(), user=user_dicts[2], activity_type='Running', duration=timedelta(hours=1, minutes=30)),
            Activity(_id=ObjectId(), user=user_dicts[3], activity_type='Strength', duration=timedelta(minutes=30)),
            Activity(_id=ObjectId(), user=user_dicts[4], activity_type='Swimming', duration=timedelta(hours=1, minutes=15)),
        ]
        Activity.objects.bulk_create(activities)

        # Create leaderboard entries
        leaderboard_entries = [
            Leaderboard(_id=ObjectId(), user=user_dicts[0], score=100),
            Leaderboard(_id=ObjectId(), user=user_dicts[1], score=90),
            Leaderboard(_id=ObjectId(), user=user_dicts[2], score=95),
            Leaderboard(_id=ObjectId(), user=user_dicts[3], score=85),
            Leaderboard(_id=ObjectId(), user=user_dicts[4], score=80),
        ]
        ...
        self.stdout.write(self.style.SUCCESS('Successfully populated the database with comprehensive test data!'))
